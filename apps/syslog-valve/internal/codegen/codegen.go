...
func Generate(g *graph.Graph, version string, shares Shares) (string, error) {
	...
	g = g.Enabled() // toggled-off nodes stay in the graph but not the config
	...
	for _, n := range g.Nodes {
		...
		case graph.TypeSource:
			...
			// udp+tcp listens on both: one source block, two drivers.
			transports := []string{n.Config.Transport}
			if n.Config.Transport == "udp+tcp" {
				transports = []string{"udp", "tcp"}
			}
			fmt.Fprintf(&b, "\n# source %s\nsource s_%s {\n", name(n), graph.Ident(n.ID))
			for _, tr := range transports {
				fmt.Fprintf(&b, "    network(ip(%q) transport(%q) port(%d)%s);\n", bind, tr, n.Config.Port, tls)
			}
			b.WriteString("};\n")
